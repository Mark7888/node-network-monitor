...
func (p *PostgresDB) Migrate() error {
	...
	if err := p.bootstrapGooseIfNeeded(); err != nil {
...
func (p *PostgresDB) bootstrapGooseIfNeeded() error {
	ctx := context.Background()

	// Check whether the nodes table already exists.
	var tableCount int
	err := p.db.QueryRowContext(ctx,
		"SELECT count(*) FROM information_schema.tables WHERE table_schema='public' AND table_name='nodes'",
	).Scan(&tableCount)
	if err != nil {
	...
	logger.Log.Info("Pre-goose PostgreSQL database detected: fast-forwarding migration history",
